...
var DYNAMIC_BASE = 170867744, DYNAMICTOP_PTR = 165624704;
...
var ASM_CONSTS = {
	...
	52915: function ($0, $1, $2, $3, $4) {
		...
		var url = hot_x === 0 && hot_y === 0
			...
			: "url(" + canvas.toDataURL() + ") " + hot_x + " " + hot_y +
				", auto";
	...
	55521: function () {
		if (typeof AudioContext !== "undefined") return 1;
		else if (typeof webkitAudioContext !== "undefined") return 1;
		return 0;
	},
	55687: function () {
		if (
		...
		else if (typeof navigator.webkitGetUserMedia !== "undefined") return 1;
		return 0;
	},
	55913: function ($0) {
	...
	56536: function ($0, $1, $2, $3) {
		...
		var have_microphone = function (stream) {
			...
			SDL2.capture.mediaStreamNode = SDL2.audioContext
				.createMediaStreamSource(stream);
			SDL2.capture.scriptProcessorNode = SDL2.audioContext
	...
	58598: function ($0, $1) {
		...
		for (var c = 0; c < numChannels; ++c) {
			var channelData = SDL2.capture.currentCaptureBuffer.getChannelData(
				c,
			);
			if (channelData.length != $1) {
				throw "Web Audio capture buffer length mismatch! Destination size: " +
					channelData.length + " samples vs expected " + $1 +
					" samples!";
			}
			if (numChannels == 1) {
				for (var j = 0; j < $1; ++j) {
	...
	59203: function ($0, $1) {
		...
		for (var c = 0; c < numChannels; ++c) {
			var channelData = SDL2.audio.currentOutputBuffer["getChannelData"](
				c,
			);
			if (channelData.length != $1) {
	...
	59683: function ($0) {
		...
		if (
			SDL2.audioContext !== undefined && SDL2.audio === undefined &&
			SDL2.capture === undefined
		) {
...
function _emscripten_set_main_loop(
	...
	} else {browserIterationFunc = function () {
			...
			function LazyUint8Array_cacheLength() {
				...
				var doXHR = function (from, to) {
					...
					if (xhr.response !== undefined) {
						return new Uint8Array(xhr.response || []);
					} else {return intArrayFromString(
							xhr.responseText || "",
							true,
						);}
				};
...
var GL = {
	...
	generateTempBuffers: function (quads, context) {
		...
		context.tempVertexBufferCounters1.length =
			context
				.tempVertexBufferCounters2.length =
				largestIndex + 1;
		context.tempVertexBuffers1 = [];
		context.tempVertexBuffers2 = [];
		context.tempVertexBuffers1.length =
			context.tempVertexBuffers2.length =
				largestIndex + 1;
...
function _eglSwapBuffers() {
	...
	else if (!Module.ctx) EGL.setErrorCode(12290);
	else if (Module.ctx.isContextLost()) EGL.setErrorCode(12302);
	else {
		EGL.setErrorCode(12288);
		return 1;
	}
...
function _JSEvents_resizeCanvasForFullscreen(target, strategy) {
	...
	if (strategy.scaleMode == 3) {
		...
		if (
			cssWidth * windowedRttHeight < windowedRttWidth * cssHeight
		) {
...
function __requestPointerLock(target) {
	...
	else if (target.msRequestPointerLock) target.msRequestPointerLock();
	else {if (
...
function __fillGamepadEventData(eventStruct, e) {
	...
	for (var i = 0; i < e.buttons.length; ++i) {
		...
		} else HEAP32[eventStruct + i * 4 + 1040 >> 2] = e.buttons[i] == 1;
	}
...
function _emscripten_glCompressedTexImage2D(
	...
	if (GL.currentContext.version >= 2) {
		...
		} else {GLctx["compressedTexImage2D"](
				target,
				level,
				internalFormat,
				width,
...
function _emscripten_glCompressedTexSubImage2D(
	...
	if (GL.currentContext.version >= 2) {
		...
		} else {GLctx["compressedTexSubImage2D"](
				target,
				level,
				xoffset,
				yoffset,
...
function _emscripten_glGetActiveUniformBlockiv(
	...
	switch (pname) {
		...
		default:
			...
			if (!result) return;
			if (typeof result == "number") HEAP32[params >> 2] = result;
...
function _emscripten_glGetProgramiv(program, pname, p) {
	...
	} else {HEAP32[p >> 2] = GLctx.getProgramParameter(
			GL.programs[program],
			pname,
		);}
}
...
function _emscripten_set_visibilitychange_callback_on_thread(
	...
	if (!specialHTMLTargets[1]) return -4;
...
var ___tm_current = 165624720;
var ___tm_timezone = (stringToUTF8("GMT", 165624768, 4), 165624768);
function _tzset() {
...
function _signal(sig, func) {
	if (sig == 14) __sigalrm_handler = func;
	else {}
	return 0;
}
...
function _strftime(s, maxsize, format, tm) {
	...
	var EXPANSION_RULES_2 = {
		...
		"%V": function (date) {
			...
			} else {daysDifference = date.tm_yday + 1 -
					firstWeekStartThisYear.getDate();}
...
var asmLibraryArg = {
	...
	"jg": _emscripten_glActiveTexture,
	"ig": _emscripten_glAttachShader,
	"ld": _emscripten_glBeginQuery,
	"Ag": _emscripten_glBeginQueryEXT,
	"Qc": _emscripten_glBeginTransformFeedback,
	"gg": _emscripten_glBindAttribLocation,
	"fg": _emscripten_glBindBuffer,
	"Nc": _emscripten_glBindBufferBase,
	"Oc": _emscripten_glBindBufferRange,
	"eg": _emscripten_glBindFramebuffer,
	"dg": _emscripten_glBindRenderbuffer,
	"Vb": _emscripten_glBindSampler,
	"cg": _emscripten_glBindTexture,
	"Nb": _emscripten_glBindTransformFeedback,
	"Vc": _emscripten_glBindVertexArray,
	"rg": _emscripten_glBindVertexArrayOES,
	"bg": _emscripten_glBlendColor,
	"ag": _emscripten_glBlendEquation,
	"$f": _emscripten_glBlendEquationSeparate,
	"_f": _emscripten_glBlendFunc,
	...
	"Uf": _emscripten_glClearColor,
	"Tf": _emscripten_glClearDepthf,
	...
	"nd": _emscripten_glDeleteQueries,
	"Cg": _emscripten_glDeleteQueriesEXT,
	...
	"qg": _emscripten_glDeleteVertexArraysOES,
	...
	"wb": _emscripten_glDrawArraysInstancedARB,
	"yd": _emscripten_glDrawArraysInstancedEXT,
	"xb": _emscripten_glDrawArraysInstancedNV,
	...
	"lg": _emscripten_glDrawElementsInstancedANGLE,
	"ub": _emscripten_glDrawElementsInstancedARB,
	"vb": _emscripten_glDrawElementsInstancedEXT,
	...
	"Dg": _emscripten_glGenQueriesEXT,
	...
	"pg": _emscripten_glGenVertexArraysOES,
	...
	"gd": _emscripten_glGetBufferPointerv,
	...
	"id": _emscripten_glGetQueryObjectuiv,
	"vg": _emscripten_glGetQueryObjectuivEXT,
	"jd": _emscripten_glGetQueryiv,
	...
	"md": _emscripten_glIsQuery,
	"Bg": _emscripten_glIsQueryEXT,
	...
	"og": _emscripten_glIsVertexArrayOES,
	...
	"qe": _emscripten_glShaderBinary,
	"pe": _emscripten_glShaderSource,
	"oe": _emscripten_glStencilFunc,
	"ne": _emscripten_glStencilFuncSeparate,
	"me": _emscripten_glStencilMask,
	"le": _emscripten_glStencilMaskSeparate,
	"ke": _emscripten_glStencilOp,
	"je": _emscripten_glStencilOpSeparate,
	"ie": _emscripten_glTexImage2D,
	"td": _emscripten_glTexImage3D,
	"he": _emscripten_glTexParameterf,
	"ge": _emscripten_glTexParameterfv,
	"fe": _emscripten_glTexParameteri,
	"ee": _emscripten_glTexParameteriv,
	"Cb": _emscripten_glTexStorage2D,
	"Bb": _emscripten_glTexStorage3D,
	"de": _emscripten_glTexSubImage2D,
	"sd": _emscripten_glTexSubImage3D,
	"Mc": _emscripten_glTransformFeedbackVaryings,
	"ce": _emscripten_glUniform1f,
	"be": _emscripten_glUniform1fv,
	"ae": _emscripten_glUniform1i,
	"$d": _emscripten_glUniform1iv,
	"Bc": _emscripten_glUniform1ui,
	"xc": _emscripten_glUniform1uiv,
	"_d": _emscripten_glUniform2f,
	...
	"Od": _emscripten_glUniformMatrix2fv,
	"ed": _emscripten_glUniformMatrix2x3fv,
	"cd": _emscripten_glUniformMatrix2x4fv,
	...
	"kg": _emscripten_glVertexAttribDivisorANGLE,
	"yb": _emscripten_glVertexAttribDivisorARB,
	"zd": _emscripten_glVertexAttribDivisorEXT,
	...
	"za": _emscripten_set_gamepadconnected_callback_on_thread,
	"ya": _emscripten_set_gamepaddisconnected_callback_on_thread,
	...
	"Ra": _emscripten_set_mouseleave_callback_on_thread,
...
var _neil_serialize = Module["_neil_serialize"] = function () {
	return (_neil_serialize = Module["_neil_serialize"] = Module["asm"]["bh"])
		.apply(null, arguments);
};
var _neil_toast_message = Module["_neil_toast_message"] = function () {
	return (_neil_toast_message =
		Module["_neil_toast_message"] =
			Module["asm"]["ch"]).apply(null, arguments);
};
var _neil_export_eep = Module["_neil_export_eep"] = function () {
	return (_neil_export_eep = Module["_neil_export_eep"] = Module["asm"]["dh"])
		.apply(null, arguments);
};
var _neil_export_sra = Module["_neil_export_sra"] = function () {
	return (_neil_export_sra = Module["_neil_export_sra"] = Module["asm"]["eh"])
		.apply(null, arguments);
};
var _neil_export_fla = Module["_neil_export_fla"] = function () {
	return (_neil_export_fla = Module["_neil_export_fla"] = Module["asm"]["fh"])
		.apply(null, arguments);
};
var _neil_unserialize = Module["_neil_unserialize"] = function () {
	return (_neil_unserialize =
...
var _neilGetSoundBufferResampledAddress =
	Module["_neilGetSoundBufferResampledAddress"] = function () {
		return (_neilGetSoundBufferResampledAddress =
...
var _neilGetAudioWritePosition = Module["_neilGetAudioWritePosition"] =
	function () {
		return (_neilGetAudioWritePosition =
			Module["_neilGetAudioWritePosition"] =
...
var _neil_send_mobile_controls = Module["_neil_send_mobile_controls"] =
	function () {
		return (_neil_send_mobile_controls =
			Module["_neil_send_mobile_controls"] =
				Module["asm"]["nh"]).apply(null, arguments);
	};
var _neil_set_buffer_remaining = Module["_neil_set_buffer_remaining"] =
	function () {
		return (_neil_set_buffer_remaining =
			Module["_neil_set_buffer_remaining"] =
				Module["asm"]["oh"]).apply(null, arguments);
	};
var _neil_set_endframe_callback = Module["_neil_set_endframe_callback"] =
	function () {
		return (_neil_set_endframe_callback =
...
var _neil_set_double_speed = Module["_neil_set_double_speed"] = function () {
	return (_neil_set_double_speed =
		Module["_neil_set_double_speed"] =
...
var __get_daylight = Module["__get_daylight"] = function () {
	return (__get_daylight = Module["__get_daylight"] = Module["asm"]["th"])
		.apply(null, arguments);
};
var __get_timezone = Module["__get_timezone"] = function () {
	return (__get_timezone = Module["__get_timezone"] = Module["asm"]["uh"])
		.apply(null, arguments);
};
...
var stackSave = Module["stackSave"] = function () {
	return (stackSave = Module["stackSave"] = Module["asm"]["wh"]).apply(
		null,
		arguments,
	);
};
var stackRestore = Module["stackRestore"] = function () {
	return (stackRestore = Module["stackRestore"] = Module["asm"]["xh"]).apply(
		null,
		arguments,
	);
};
var stackAlloc = Module["stackAlloc"] = function () {
	return (stackAlloc = Module["stackAlloc"] = Module["asm"]["yh"]).apply(
		null,
		arguments,
	);
};
var dynCall_vi = Module["dynCall_vi"] = function () {
	return (dynCall_vi = Module["dynCall_vi"] = Module["asm"]["zh"]).apply(
		null,
		arguments,
	);
};
var dynCall_viiii = Module["dynCall_viiii"] = function () {
	return (dynCall_viiii = Module["dynCall_viiii"] = Module["asm"]["Ah"])
		.apply(null, arguments);
};
var dynCall_iii = Module["dynCall_iii"] = function () {
	return (dynCall_iii = Module["dynCall_iii"] = Module["asm"]["Bh"]).apply(
		null,
		arguments,
	);
};
...
var dynCall_iiiii = Module["dynCall_iiiii"] = function () {
	return (dynCall_iiiii = Module["dynCall_iiiii"] = Module["asm"]["Dh"])
		.apply(null, arguments);
};
var dynCall_iidii = Module["dynCall_iidii"] = function () {
	return (dynCall_iidii = Module["dynCall_iidii"] = Module["asm"]["Eh"])
		.apply(null, arguments);
};
var dynCall_vii = Module["dynCall_vii"] = function () {
	return (dynCall_vii = Module["dynCall_vii"] = Module["asm"]["Fh"]).apply(
		null,
		arguments,
	);
};
...
var dynCall_v = Module["dynCall_v"] = function () {
	return (dynCall_v = Module["dynCall_v"] = Module["asm"]["Hh"]).apply(
		null,
		arguments,
	);
};
var dynCall_ii = Module["dynCall_ii"] = function () {
	return (dynCall_ii = Module["dynCall_ii"] = Module["asm"]["Ih"]).apply(
		null,
		arguments,
	);
};
var dynCall_jiji = Module["dynCall_jiji"] = function () {
	return (dynCall_jiji = Module["dynCall_jiji"] = Module["asm"]["Jh"]).apply(
		null,
		arguments,
	);
};
var dynCall_ji = Module["dynCall_ji"] = function () {
	return (dynCall_ji = Module["dynCall_ji"] = Module["asm"]["Kh"]).apply(
		null,
		arguments,
	);
};
...
var dynCall_iiiiiidii = Module["dynCall_iiiiiidii"] = function () {
	return (dynCall_iiiiiidii =
		Module["dynCall_iiiiiidii"] =
			Module["asm"]["Nh"]).apply(null, arguments);
};
var dynCall_iiiiiiiii = Module["dynCall_iiiiiiiii"] = function () {
	return (dynCall_iiiiiiiii =
		Module["dynCall_iiiiiiiii"] =
			Module["asm"]["Oh"]).apply(null, arguments);
};
var dynCall_viiiiiii = Module["dynCall_viiiiiii"] = function () {
	return (dynCall_viiiiiii = Module["dynCall_viiiiiii"] = Module["asm"]["Ph"])
		.apply(null, arguments);
};
var dynCall_viiiiiiiiiii = Module["dynCall_viiiiiiiiiii"] = function () {
	return (dynCall_viiiiiiiiiii =
		Module["dynCall_viiiiiiiiiii"] =
			Module["asm"]["Qh"]).apply(null, arguments);
};
var dynCall_iiiiiiii = Module["dynCall_iiiiiiii"] = function () {
	return (dynCall_iiiiiiii = Module["dynCall_iiiiiiii"] = Module["asm"]["Rh"])
		.apply(null, arguments);
};
...
var dynCall_viiiii = Module["dynCall_viiiii"] = function () {
	return (dynCall_viiiii = Module["dynCall_viiiii"] = Module["asm"]["Th"])
		.apply(null, arguments);
};
var dynCall_iiiiiii = Module["dynCall_iiiiiii"] = function () {
	return (dynCall_iiiiiii = Module["dynCall_iiiiiii"] = Module["asm"]["Uh"])
		.apply(null, arguments);
};
var dynCall_viiiiii = Module["dynCall_viiiiii"] = function () {
	return (dynCall_viiiiii = Module["dynCall_viiiiii"] = Module["asm"]["Vh"])
		.apply(null, arguments);
};
var dynCall_iidiiii = Module["dynCall_iidiiii"] = function () {
	return (dynCall_iidiiii = Module["dynCall_iidiiii"] = Module["asm"]["Wh"])
		.apply(null, arguments);
};
var dynCall_viijii = Module["dynCall_viijii"] = function () {
	return (dynCall_viijii = Module["dynCall_viijii"] = Module["asm"]["Xh"])
		.apply(null, arguments);
};
var dynCall_iiiiij = Module["dynCall_iiiiij"] = function () {
	return (dynCall_iiiiij = Module["dynCall_iiiiij"] = Module["asm"]["Yh"])
		.apply(null, arguments);
};
var dynCall_iiiiid = Module["dynCall_iiiiid"] = function () {
	return (dynCall_iiiiid = Module["dynCall_iiiiid"] = Module["asm"]["Zh"])
		.apply(null, arguments);
};
var dynCall_iiiiijj = Module["dynCall_iiiiijj"] = function () {
	return (dynCall_iiiiijj = Module["dynCall_iiiiijj"] = Module["asm"]["_h"])
		.apply(null, arguments);
};
var dynCall_iiiiiijj = Module["dynCall_iiiiiijj"] = function () {
	return (dynCall_iiiiiijj = Module["dynCall_iiiiiijj"] = Module["asm"]["$h"])
		.apply(null, arguments);
};
var dynCall_vffff = Module["dynCall_vffff"] = function () {
	return (dynCall_vffff = Module["dynCall_vffff"] = Module["asm"]["ai"])
		.apply(null, arguments);
};
var dynCall_vf = Module["dynCall_vf"] = function () {
	return (dynCall_vf = Module["dynCall_vf"] = Module["asm"]["bi"]).apply(
		null,
		arguments,
	);
};
var dynCall_viiiiiiii = Module["dynCall_viiiiiiii"] = function () {
	return (dynCall_viiiiiiii =
...
var dynCall_vff = Module["dynCall_vff"] = function () {
	return (dynCall_vff = Module["dynCall_vff"] = Module["asm"]["ei"]).apply(
		null,
		arguments,
	);
};
var dynCall_vfi = Module["dynCall_vfi"] = function () {
	return (dynCall_vfi = Module["dynCall_vfi"] = Module["asm"]["fi"]).apply(
		null,
		arguments,
	);
};
var dynCall_viif = Module["dynCall_viif"] = function () {
	return (dynCall_viif = Module["dynCall_viif"] = Module["asm"]["gi"]).apply(
		null,
		arguments,
	);
};
var dynCall_vif = Module["dynCall_vif"] = function () {
	return (dynCall_vif = Module["dynCall_vif"] = Module["asm"]["hi"]).apply(
		null,
		arguments,
	);
};
var dynCall_viff = Module["dynCall_viff"] = function () {
	return (dynCall_viff = Module["dynCall_viff"] = Module["asm"]["ii"]).apply(
		null,
		arguments,
	);
};
var dynCall_vifff = Module["dynCall_vifff"] = function () {
	return (dynCall_vifff = Module["dynCall_vifff"] = Module["asm"]["ji"])
		.apply(null, arguments);
};
var dynCall_viffff = Module["dynCall_viffff"] = function () {
	return (dynCall_viffff = Module["dynCall_viffff"] = Module["asm"]["ki"])
		.apply(null, arguments);
};
var dynCall_viiiiiiiiii = Module["dynCall_viiiiiiiiii"] = function () {
	return (dynCall_viiiiiiiiii =
		Module["dynCall_viiiiiiiiii"] =
			Module["asm"]["li"]).apply(null, arguments);
};
var dynCall_viifi = Module["dynCall_viifi"] = function () {
	return (dynCall_viifi = Module["dynCall_viifi"] = Module["asm"]["mi"])
		.apply(null, arguments);
};
var dynCall_iiiji = Module["dynCall_iiiji"] = function () {
	return (dynCall_iiiji = Module["dynCall_iiiji"] = Module["asm"]["ni"])
		.apply(null, arguments);
};
...
var dynCall_iiiif = Module["dynCall_iiiif"] = function () {
	return (dynCall_iiiif = Module["dynCall_iiiif"] = Module["asm"]["pi"])
		.apply(null, arguments);
};
